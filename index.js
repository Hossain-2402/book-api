...
app.get('/api/category/:book_category', (req, res) => {
  const { book_category } = req.params;

  Product.find({ book_category })
    .then(result => {
      res.status(200).json(result);
    })
    .catch(err => {
...
app.get('/api/search/:book_name', (req, res) => {
  const { book_name } = req.params;

  Product.find({ book_name })
    .then(result => {
      res.status(200).json(result);
    })
    .catch(err => {
